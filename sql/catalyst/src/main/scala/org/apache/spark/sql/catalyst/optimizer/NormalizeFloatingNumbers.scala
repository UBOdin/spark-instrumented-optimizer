...
object NormalizeFloatingNumbers extends Rule[LogicalPlan] {
  ...
  def apply(plan: LogicalPlan): LogicalPlan = plan match {
    case _ =>
      CustomLogger.logTransformTime("stopTheClock : Transform NormalizeFloatingNumbers")
      {
      ...
      case w: Window if w.partitionSpec.exists(p => needNormalize(p)) =>
        ...
        CustomLogger.logMatchTime("stopTheClock : Match 1 NormalizeFloatingNumbers", true)
        {
      ...
      case j @ ExtractEquiJoinKeys(_, leftKeys, rightKeys, condition, _, _, _)
        ...
        CustomLogger.logMatchTime("stopTheClock : Match 2 NormalizeFloatingNumbers", true)
        {
