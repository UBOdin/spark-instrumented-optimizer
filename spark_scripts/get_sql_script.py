...
with open(createtablefile) as fp:
	...
	for line in lines:
		if "CREATE TABLE" in line:
			...
			csv_file = table_path+table.lower()+".tbl"
...
file_names = []
sql_commands = []
for path in pathlib.Path(tpch_queries_path).iterdir():
    if path.is_file():
        current_file = open(path, "r")
        file_name = current_file.name
        file_name = file_name.replace(tpch_queries_path+"/","")
...
for i in range(0,22):
	...
	scala_write_file.writelines("println(\"Run SQL Query "+file_names[i]+" \")\n")
	for j in range(0,8):
...
# output_command = ".scala > /home/csgrad/dbalakri/Spark_scratch/tpch/tpch_output_1/output_"
...
# shell_file = open(r"../tpch/tpch_spark_shell_script_1.sh","w")
